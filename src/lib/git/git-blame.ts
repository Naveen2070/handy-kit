...
import type { BlameResult, GitBlameFlags } from "../types/utils.js";
import {
  getFilesToAnalyze,
  parseBlameOutput,
  formatOutput,
  formatDetailedSummaryOutput,
 ...
 */
export function gitBlameSummary(flags: GitBlameFlags = {}): void {
  ...
  if (exportPath && !flags.file && !summary) {
    console.warn(
      colorize(
        "--export flag only applicable when --file and --summary are specified; ignoring --export.",
        "yellow"
      )
    );
  ...
  if (exportPath && brief) {
    console.warn(
      colorize(
        "--export flag is not compatible with --brief; ignoring --export.",
        "yellow"
      )
    );
    exportPath = undefined;
  ...
  if (brief && !flags.file) {
    console.warn(
      colorize(
        "--brief flag only applicable when --file is specified; ignoring --brief.",
        "yellow"
      )
    );
  }
  ...
  const results: Record<string, BlameResult | undefined> = {};
  const authorSummaries: Record<
    string,
  ...
  for (const file of files) {
    try {
      ...
      const { authors, briefOutput } = parseBlameOutput(
        blameOutput,
        brief && !!flags.file
      );
  ...
  if (exportPath) {
    ...
    if (summary) {
      ...
      mdContent += generateMarkdownResults(
        results as Record<string, BlameResult>
      );
    }

    try {
      writeFileSync(
        exportPath.toString() === "true" ? "git-blame.md" : exportPath,
        mdContent,
        { encoding: "utf-8" }
      );
      console.log(
