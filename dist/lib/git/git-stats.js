...
import { exportReport, printTemplate, renderChart, } from "../utils/common/index.js";
/**
 * gitStats.ts
 *
 ...
 * bottom.
 */
export async function gitStats(flags) {
    /**
     ...
     */
    if (flags.since)
        command += ` --since="${flags.since}"`;
    if (flags.author)
        command += ` --author="${flags.author}"`;
    /**
