 ...
 */
export function getFilesToAnalyze(fileFlag) {
    if (fileFlag) {
 ...
 */
export function parseBlameOutput(blameOutput, brief) {
    const authors = {};
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (brief && line.startsWith("    lineno ")) {
            ...
            currentLineNum = parseInt(linenoStr, 10);
        }
 ...
 */
export function formatOutput(file, total, contributors, briefOutput, outputJSON) {
    if (outputJSON) {
 ...
 */
export function formatDetailedSummaryOutput(authorSummaries) {
    const sortedAuthors = Object.entries(authorSummaries).sort((a, b) => b[1].total - a[1].total);
 ...
 */
export function generateMarkdownResults(results) {
 ...
 * @returns markdown string
 */
export function generateMarkdownSummary(authorSummaries) {
...
//# sourceMappingURL=blameUtils.js.map