 ...
 */
export function displayOutdatedPackages(outdated) {
    ...
    for (const [pkg, info] of entries) {
        ...
        if (info.location)
            console.log(`    • Location:    ${info.location}`); // location of the dependency in the project
        if (info["depended by"])
            console.log(`    • Depended by: ${info["depended by"]}`); // packages that depend on this package
        if (info.type)
            console.log(`    • Type:        ${info.type}`); // type of the dependency (e.g. devDependency, optionalDependency)
        console.log("");
    }
}
//# sourceMappingURL=outdatedUtils.js.map