 ...
 */
export async function handleAuditDepsCommand(_, flags) {
    ...
    if (exportPath) {
        try {
            await fs.writeFile(exportPath.toString(), JSON.stringify(auditData, null, 2));
    ...
    function printVuln(name, vuln) {
        ...
        if (vuln.fixAvailable) {
            ...
            if (typeof fix === "object") {
                console.log(`  Fix:      Upgrade ${fix.name} to version ${fix.version}${fix.isSemVerMajor ? " (major)" : ""}`);
            }
            else {
                console.log(`  Fix:      ${fix}`);
            }
    ...
    for (const severity of severityOrder) {
        const filtered = Object.entries(vulns).filter(([_, vuln]) => (vuln.severity || "unknown") === severity);
